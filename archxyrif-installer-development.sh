...
safe_disk_cleanup() {
    ...
    vgchange -an || true
    for lv in $(lsblk -tpfs NAME "$DEV" | grep -E '^.*--.*$' || true); do
    ...
    for map in $(lsblk -tpfs NAME,TYPE | awk '$2=="crypt"{print $1}'); do
        local backing
    ...
    for p in $(lsblk -tpfs -o NAME,MOUNTPOINT "$DEV" | awk '$2!=""{print $1}' | tac); do
        local part="/dev/$p"
    ...
    for part in $(lsblk -tpfs -o NAME "$DEV" | tail -n +2); do
...
ask_partition_sizes() {
    ...
    while true; do
        lsblk -tpfs NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT "$DEV"
...
install_grub() {
    ...
    mapfile -t luks_lines < <(lsblk -tpfs NAME,TYPE -nr | grep -E "crypt|luks")
    ...
    mapfile -t fs_lines < <(lsblk -tpfs MOUNTPOINT,FSTYPE -nr /mnt | grep -v '^$')
...
quick_partition() {
    ...
    lsblk -tpfs NAME,SIZE,MODEL,TYPE
    while true; do
