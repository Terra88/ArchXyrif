...
partition_disk() {
    ...
    echo -e "\n🛈 Creating GPT partition table on $DEV..."
...
format_and_mount() {
    ...
    if [[ "$MODE" == "UEFI" ]]; then
        ...
        P_BIOS_GRUB="${PARTS[0]}"   # tiny 1MiB partition, DO NOT format or mount
        P_BOOT="${PARTS[1]}"
        P_ROOT="${PARTS[2]}"
    ...
    # ---------------------------------------------------------------------
    if [[ "$MODE" == "UEFI" ]]; then
        echo "→ Mounting EFI at /mnt/boot/efi..."
        ...
        # NOTE: BIOS_GRUB does not need mounting or formatting
    fi
